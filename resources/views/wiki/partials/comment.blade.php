								...
								<div class="comment-actions">
									<ul class="list-unstyled list-inline">
										<li>
											...
											@endif
											@endforeach
											<i class="fa fa-spinner fa-spin fa-lg fa-fw" id="spinner"></i> <a href="#" id="like-comment" data-comment-id="{{ $comment->id }}"><i class="fa fa-thumbs-o-up fa-fw" style="font-size: 14px;"></i> {{ $userLikeComment ? trans('function.Unlike') : trans('function.Like') }}</a>
										</li>
        ...
        <div class="media">
            ...
            <div class="media-body">
                <div class="form-group {{ $errors->has('comment') ? 'has-error' : '' }}" style="margin-bottom: 0px;">
                    <textarea name="comment" class="form-control" id="comment-input-textarea" placeholder="@lang('message.WriteComment')" style="margin-bottom: 13px;"></textarea>
                    @if($errors->has('comment'))
					<p class="help-block has-error" style="width: 230px; margin-bottom: 0; position: absolute;">{{ $errors->first('comment') }}</p>
                    @endif
                    <button type="submit" class="btn btn-primary pull-right" style="border-radius: 3px;">@lang('function.Submit')</button>
                    <div class="clearfix"></div>
                </div>
            </div>
        </div>
