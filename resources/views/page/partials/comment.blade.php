...
<div class="panel panel-default" style="margin-bottom: 20px; border: 1px solid #eee;">
		...
		<div class="pull-right">
			<ul class="list-unstyled list-inline" style="margin-bottom: 0;">
				<li class="page-like-con">
					...
					<a href="#" id="like-page" data-page="{{ $page->slug }}"><i class="fa fa-star-o fa-fw" style="font-size: 16px;" data-toggle="tooltip" data-placement="top" title="{{ $isUserLikePage ? trans('function.Unlike') : trans('function.Like') }}" style="margin-right: 2px;"></i></a> <span class="label" id="likes-counter" style="color: #9c9c9c; font-size: 12px; font-weight: 600; padding: 0px; margin-left: 2px;">{{ $page->likes->count() }}</span>
				</li>
				<li>
	...
	<div class="panel-body wiki-comments-con">
		<div class="comments">
						...
						<div class="comment-body-con">
							<div class="comment-body-inner">
								<div class="comment-body">
									<p class="comment-content" data-comment-content="{{ html_entity_decode(strip_tags($comment->content), null, 'utf-8') }}">{!! (new Emoji)->render(nl2br($comment->content)) !!}</p>
								</div>
								<div class="comment-actions">
									<ul class="list-unstyled list-inline">
										<li>
											...
											@endforeach
											<i class="fa fa-spinner fa-spin fa-lg fa-fw" id="spinner"></i>
											<a href="#" id="like-comment" data-comment-id="{{ $comment->id }}"><i class="fa fa-thumbs-o-up fa-fw" style="font-size: 14px;"></i> {{ $userLikeComment ? trans('function.Unlike') : trans('function.Like') }}</a>
										</li>
										@if($comment->user_id === Auth::user()->id)
										<li>
											<a href="#" id="edit-comment"><i class="fa fa-pencil fa-fw" style="font-size: 14px;"></i> @lang('function.Edit')</a>
										</li>
										<li>
											<a href="#" id="delete-comment" data-comment-id="{{ $comment->id }}"><i class="fa fa-trash-o fa-fw" style="font-size: 14px;"></i> @lang('function.Delete')</a>
										</li>
										@endif
									</ul>
								</div>
			...
			@endforeach
			@else 
			<h1 class="nothing-found">@lang('message.Nothingfound')</h1>
			@endif
		</div>
	</div>
</div>
...
<div class="wiki-comment-form">
    <form action="{{ route('pages.comments.store', [$team->slug, $space->slug, $wiki->slug, $page->slug]) }}" method="POST">    
        <div class="media">
            ...
            <div class="media-body">
                <div class="form-group {{ $errors->has('comment') ? 'has-error' : '' }}" style="margin-bottom: 13px;">
                    <textarea name="comment" class="form-control" id="comment-input-textarea" placeholder="@lang('message.WriteComment')"></textarea>
        ...
        <input type="submit" class="btn btn-primary pull-right" value="@lang('function.Submit')">
        <div class="clearfix"></div>
    </form>
</div>