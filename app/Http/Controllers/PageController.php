...
class PageController extends Controller
{
	...
	public function destroy(Team $team, Space $space, Wiki $wiki, Page $page)
	{
		...
		return redirect()->route('wikis.show', [$team->slug, $wiki->space->slug, $wiki->slug])->with([
					'alert' => trans('message.SuccessfullyDeleted', ['name' => trans('menu.Page')]),
					'alert_type' => 'success',
		]);
	}
	...
	public function store(Team $team, Space $space, Wiki $wiki)
	{
		...
		return redirect()->route('pages.show', [$team->slug, $space->slug, $wiki->slug, $page->slug])->with([
					'alert' => trans('message.SuccessfullyCreated', ['name' => trans('menu.Page')]),
					'alert_type' => 'success',
		]);
	}
	...
	{
		$pages = $this->page->getPagesArr($wiki->id);
	...
	public function update(Team $team, Space $space, Wiki $wiki, Page $page)
	{
		...
		return redirect()->route('pages.show', [$team->slug, $space->slug, $wiki->slug, $page->slug])->with([
					'alert' => trans('message.SuccessfullyUpdated', ['name' => trans('menu.Page')]),
					'alert_type' => 'success',
		]);
	}
	...
	public function addToReadList(Team $team, Space $space, Wiki $wiki, Page $page)
	{
		...
		return redirect()->back()->with([
					'alert' => trans('message.SuccessfullyAddedTo', ['name' => trans('menu.Page'), 'to' => trans('menu.ReadList')]),
					'alert_type' => 'success',
		]);
	}
	...
	public function removeFromReadList(Team $team, Space $space, Wiki $wiki, Page $page)
	{
		...
		return redirect()->back()->with([
					'alert' => trans('message.SuccessfullyDeletedFrom', ['name' => trans('menu.Page'), 'from' => trans('menu.ReadList')]),
					'alert_type' => 'success',
		]);
	}
	...
	{
		$this->validate($this->request, Page::getValidationRules($page));
		...
		return redirect()->route('pages.settings', [$team->slug, $wiki->space->slug, $wiki->slug, $this->request->get('slug')])->with([
					'alert' => trans('message.SuccessfullyUpdated', ['name' => trans('menu.Page')]),
					'alert_type' => 'success',
		]);
	}
