...
class UserController extends Controller
{
	...
	public function updateAvatar(Team $team, User $user)
	{
		...
		return redirect()->back()->with([
					'alert' => trans('message.SuccessfullyUpdated', ['name' => trans('menu.ProfilePicture')]),
					'alert_type' => 'success',
		]);
	}
	...
	public function update(Team $team, User $user)
	{
		...
		return redirect()->back()->with([
					'alert' => trans('message.SuccessfullyUpdated', ['name' => trans('menu.Profile')]),
					'alert_type' => 'success',
		]);
	}
	...
	public function deleteAccount(Team $team, User $user)
	{
		...
		return $this->logout()->with([
					'alert' => trans('message.SuccessfullyDeleted', ['name' => trans('menu.YourAccount')]),
					'alert_type' => 'success',
		]);
	}
	...
	public function sendResetLinkEmail()
	{
		$this->validate($this->request, [
			...
			'is_email_exists_in_team' => trans('message.EmailDoestExistsInThisTeam'),
		]);

        $team = Team::where('id' , $this->request->get('team_id'))->first();

		$token = str_rot13(base64_encode(str_rot13($team->name . $this->request->get('email'))));
		...
		return redirect()->back()->with([
					'alert' => trans('message.PleaseCheckInbox'),
		]);
	}
	...
	public function sendResetPasswordEmail($email, $token)
	{
		Mail::send('mails.reset-password', ['email' => $email, 'token' => $token], function ($message) use ($email) {
			$message->from('developer@example.com', 'opus knowledge base');
	...
	public function reset($token)
	{
		...
		return redirect()->route('team.login')->with([
					'alert' => trans('message.Successfullychanged', ['name' => trans('menu.Password')]),
					'alert_type' => 'success',
		]);
	}
