...
class Page extends Node
{
	 ...
	 */
	protected $fillable = [
		'name', 'slug', 'outline', 'description', 'position', 'parent_id', 'user_id',
	 ...
	 * @return array
	 */
	public static function getValidationRules(Page $page)
	{
		return [
			'name' => 'required|max:35',
			'slug' => [
				Rule::unique('page')->ignore($page->id),
			],
		];
	}
	...
	/**
	 * Get all the nodes|pages of a wiki with Array.
	 *
	 * @param $wikiId integer
	 * @return array
	 */
	public function getPagesArr($wikiId)
	{
		return $this->where('wiki_id', $wikiId)->with(['wiki'])->pluck('name', 'id')->toArray();
	}
